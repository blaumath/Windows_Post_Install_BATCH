...
if (Test-Path $MainPath) {
    if ($args.Count -eq 1) {
        ...
        Start-Sleep -Seconds 3
        Start-Process -FilePath "$MainPath\Install.bat" -ArgumentList "$op $op2 $op0" -NoNewWindow -RedirectStandardOutput "NUL" -RedirectStandardError "NUL" -Wait

    }elseif (($args.Count -ge 2 -and $args.Count -lt 3) -and ($args[0] -eq "/s" -and $args[1] -ne "/s")){
        $op = $args[1]
        Write-Host "MORE THAN 1 WITH /S ONLY 1 OPTION"
        Start-Sleep -Seconds 3
        Start-Process -FilePath "$MainPath\Install.bat" -ArgumentList "$op 0" -NoNewWindow -RedirectStandardOutput "NUL" -RedirectStandardError "NUL" -Wait
